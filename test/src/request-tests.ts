...
describe('Request endpoints', () => {
	...
	describe('Challenge', () => {
		...
		it('user details should return finished challenge', async () => {
			const validSolution = `const solution = (n) => { return n*n; }`;
			const response = await request(app)
				.post('/challenge/submit/' + challengeId)
				.send({
			...
			expect(detailsResponse.status).to.equal(HTTP_CODES.HTTP_OK);
			const detailsBody = detailsResponse.body;
			expect(detailsBody.rank).to.equal(1);
